...
class Storage:
    def __init__(self):
        ...
        This constructor sets up two dictionaries to hold prompts and
        collections, associating string keys with their respective Prompt
        and Collection objects.

        Attributes:
            _prompts (Dict[str, Prompt]): A dictionary to store prompt
                objects, keyed by their unique identifiers.
            _collections (Dict[str, Collection]): A dictionary to store
                collection objects, keyed by their unique identifiers.
    ...
    def create_prompt(self, prompt: Prompt) -> Prompt:
        ...
        This function takes a `Prompt` object and adds it to the internal
        storage using the prompt's unique identifier. It returns the stored
        `Prompt` object.
        ...
        Usage examples:
            ...
            stored_prompt = storage_instance.create_prompt(
                prompt_instance)
        """
    ...
    def get_all_prompts(self) -> List[Prompt]:
        ...
        Returns:
            List[Prompt]: A list containing all Prompt objects stored in
                the instance.
    ...
    def update_prompt(
        ...
        This method updates the prompt identified by `prompt_id` with new
        information provided in the `prompt` parameter. If the
        `prompt_id` does not exist in the storage, the method returns None.

        Args:
            prompt_id (str): The unique identifier for the prompt to update.
            prompt (Prompt): The new prompt data to replace the existing
                prompt.

        Returns:
            Optional[Prompt]: The updated prompt if the `prompt_id` exists,
                otherwise None.
    ...
    def create_collection(self, collection: Collection) -> Collection:
        ...
        Args:
            collection (Collection): The collection object to be
                stored. It must have a unique `id` property which will
                be used as the storage key.

        Returns:
            Collection: The same collection object that was input,
            confirming that it has been successfully stored.

        Usage Example:
            collection = Collection(id='123', name='Sample',
                data=[])
            storage = Storage()
    ...
    def get_collection(self, collection_id: str) -> Optional[Collection]:
        ...
        This method fetches a collection from the internal store using
        the provided collection ID. If the collection ID exists, it
        returns the corresponding `Collection` object; otherwise, it
        returns `None`.

        Args:
            collection_id (str): The unique identifier for the
                collection to retrieve.

        Returns:
            Optional[Collection]: The collection object if found,
            otherwise `None`.

        Usage example:
            collection = storage_instance.get_collection(
                "collection123")
            if collection:
    ...
    def get_all_collections(self) -> List[Collection]:
        ...
        This method compiles all collections currently stored within
        the instance and returns them as a list. It accesses the
        internal dictionary `_collections`, which maps collection
        identifiers to `Collection` objects, and converts its values
        to a list format.

        Returns:
            List[Collection]: A list containing all `Collection`
            objects stored within the instance.
    ...
    def get_prompts_by_collection(
        ...
        This method filters and returns a list of prompts that belong
        to the given collection identifier. It searches within the
        internally stored collection of prompts and extracts only those
        that match the provided collection ID.

        Args:
            collection_id (str): The unique identifier of the
                collection for which prompts are to be retrieved.

        Returns:
            List[Prompt]: A list of `Prompt` objects that belong to
            the specified collection. If no prompts are found for the
            collection ID, an empty list is returned.

        Usage Example:
            storage = Storage()
            prompts = storage.get_prompts_by_collection(
                'collection_123')
    ...
    def clear(self):
        ...
        This method is used to empty the current storage of all prompts
        and collections. It effectively resets the storage instance to
        its initial empty state.

        Usage example:
            storage = Storage()  # Initialize the storage instance
            storage.create_prompt(
                Prompt(id="1", text="Sample prompt"))
            storage.create_collection(
                Collection(id="1", name="Sample collection"))

            # Clear all stored data
            storage.clear()
            assert len(storage.get_all_prompts()) == 0  # True
            assert len(storage.get_all_collections()) == 0  # True
        """
        self._prompts.clear()
        self._collections.clear()

    def get_prompt_by_title(
    ...
    def collection_exists_by_name(self, name: str) -> bool:
        ...
        This method searches through all stored collections to determine
        if a collection with the specified name exists.

        Args:
            name (str): The name of the collection to check.

        Returns:
            bool: True if a collection with the specified name exists,
            otherwise False.

        Usage example:
            storage = Storage()
            exists = storage.collection_exists_by_name(
                "Example Collection")
            if exists:
    ...
    def get_prompt_by_id_and_collection(
        self, prompt_id: str, collection_id: str
    ) -> Optional[Prompt]:
        """Retrieve a prompt by its ID that belongs to a specific
        collection.

        This method checks if a prompt with the specified ID belongs to
        the given collection ID and returns it if found.

        Args:
            prompt_id (str): The unique identifier of the prompt to
                retrieve.
            collection_id (str): The collection ID associated with the
                prompt.

        Returns:
            Optional[Prompt]: The prompt object if it exists in the
    ...
    def get_versions_by_prompt(
        ...
        Args:
            prompt_id (str): The unique identifier of the prompt to
                retrieve versions for.

        Returns:
            List[Dict[str, str]]: A list of version data dictionaries
            for the given prompt ID.
        """
        return self._prompt_versions.get(prompt_id, [])

    def save_prompt_version(
        self, prompt_id: str, version_data: Dict[str, str]
    ) -> None:
        """Save a version of a prompt.

        Args:
            prompt_id (str): The ID of the prompt for which the version
                is being saved.
            version_data (Dict[str, str]): The version details to be
                stored.
        """
